...
const useXORSpent = (blocks?: Block[], last?: number) => {
  ...
  const getBlocksFiltered = () => {
    ...
    if (blockLimiter !== -1) {
        return allBlocks.current.filter((block) => block.blockNum >= blockLimiter);
    }
  ...
  const totalPages = useRef(Math.ceil(blocksTimeFiltered.current.length / limiter));
